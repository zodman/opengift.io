    ...
    window.taskClass = Backbone.Model.extend({
        ...
        window.taskViewClass = Backbone.View.extend({
            ...
            'initialize':function(){
                ...
                this.arPlanTimes = [[0.5,'30 мин.'],[1,'1 ч.'],[2,'2 ч.'],[3,'3 ч.'],[4,'4 ч.'],[6,'6 ч.'],[8,'8 ч.'],[10,'10 ч.'],[16,'16 ч.'],[20,'20 ч.'],[32,'32 ч.']];
            },
            'createEl':function(){
                this.$el = $('<div></div>')
            ...
            'template':function(taskInfo,params){
                ...
                if (taskInfo.planTime || taskInfo.onPlanning || taskInfo.canSetPlanTime){
                    sPlanTime += '<span class="dropdown">&nbsp;[ ~ </span>' +
                                    '<span class="dropdown">' +
                                    '<a data-toggle="dropdown" class="jsPlanTimeHolder">'
                                    +(taskInfo.planTime || 'Время (план)')
                                + '</a>';
...
//                });
            ...
            'showPauseDialog': function(){
                var t = this;
                t.$('.pause_dialog').show();
                setTimeout(function(){t.$('.pause_dialog').one('clickoutside',function(){$(this).hide();});},100);
                return false;
            },
            'showNextPopup': function(e){
                ...
                if ($popup.is(':visible')){
                    setTimeout(function(){
                        $popup.unbind('clickoutside').bind('clickoutside',function(){$(this).hide()});
                    },10);
                }
            ...
            'showResponsibleMenu':function(){
                ...
                setTimeout(function(){userList.bind('clickoutside',function(){$(this).remove();})},10);
