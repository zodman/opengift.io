    ...
    window.taskViewClass = Backbone.View.extend({
        ...
        'template': function (taskInfo, params) {
            ...
            var tags_str = taskInfo.tags.map(function(elem){
                color = "";
                if(elem.color){
                    color = "style=' background:" +elem.color +"'";
                }
            ...
            if( taskInfo.winner.avatar ) {
                ...
                users_str += '<li class="user_message winner">' +
                    ' <i class="fa fa-check winner-selector" aria-hidden="true"></i>' +
                   '<img src="'+ image +'" class="pull-left img-circle" >' +
                    '</li>';
            }

            users_str += taskInfo.users.slice(0, users_limit).map(function(elem){
                if (taskInfo.winner && taskInfo.winner.id == elem.id) return '';
            ...
            if (taskInfo.donated || taskInfo.asked) {
                ...
                oTaskContainers.$reward.append(
                    ...
                    '                        <div class="donation-progress-inner-item ' + color + '-gr' + '" aria-valuenow="' + percent + '" style="height: ' + percent + '%;"></div>' +
                    '                    </div>' +
                    '                </div>');
            }
