...
def projectDetailEdit(request, project_id):
    ...
    def recursiveTreeDraw(treeItem):
        ...
        if 'item' in treeItem:
            ...
            s += '<h4 class="float-right text-primary">'+treeItem['item'].getPercent()+'%</h4>'
            s += '<div class="progress w-100">'
            s += '<div class="progress-bar" aria-valuenow="'+treeItem['item'].getPercent()+'" style="width: '+treeItem['item'].getPercent()+'%;"></div>'
            s += '</div>'
